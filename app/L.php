...
//
	...
	foreach($lines as $line) {
		...
		if(COUNT($words) > 0) {
			...
			switch($operation) {
				...
				case "DEFINE":
					[$leftExpr, $rightToken] = splitAssigment($args);
					[$varName, $type] = expTrim(':', $leftExpr);
					//Valdiate varName
					...
					if(count($scopeTracker) == 0) {
						...
						[$assignmentRegister, $right]  = fetchExpression($rightToken);
				...
				case "SET":	//Variable Assignment
					[$leftToken, $rightToken] = splitAssigment($args);
				...
				case "ENTRY":
					if(array_key_exists($args, $stackFrame)) {
						error("Variable already exists: " . $args . "\n");
					}
					$stackFrame[$args] = [
						'name' => $args,
				...
				case "MOV":
					...
					if($REGISTERS[strtoupper($src)] && $REGISTERS[strtoupper($dest)]) {
						...
						setToken($dest, $REGISTERS[strtoupper($src)]);
					} else if($REGISTERS[$dest]) {
	...
	function fetchExpression($expr, $assignmentReg = null) {
		...
		if(count($exprMatch)) {
			...
			[$leftReg, $left] = fetchToken($leftToken, $assignmentReg);
			...
			if(array_key_exists($exprMatch['operation'], $ALU_OPS)) {
				$operation = $ALU_OPS[$exprMatch['operation']];
			} else {
				error("\nInvalid operator: " , $exprMatch['operation']);
			}

			$result[] = 'C' . $leftReg; //Compare the 2 values and save to null
			$result[] = $leftReg . $rightReg;
			$result[] = $operation . $quickMath;
			
			return [$leftReg, $left, $operation];
		} else {
			return fetchToken($expr, $assignmentReg);
		}
	}
	...
	function splitAssigment($str) {
		$segments = expTrim('=', $str);
		if(count($segments) > 0) {
			$segments = [$segments[0], implode('=', array_slice($segments, 1))];
		}
		return $segments;
	}
